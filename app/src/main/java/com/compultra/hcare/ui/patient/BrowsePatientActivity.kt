package com.compultra.hcare.ui.patient
...
class BrowsePatientActivity : AppCompatActivity() {

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_browse_patient)
        val navView: BottomNavigationView = findViewById(R.id.patient_nav_view)

        val navHostFragment =
                supportFragmentManager.findFragmentById(R.id.patient_nav_host) as NavHostFragment
        val navController = navHostFragment.navController

        val appBarConfiguration = AppBarConfiguration(
                setOf(
                        R.id.doctorPatientFragment, R.id.consultationPatientFragment,
                        R.id.prescriptionPatientFragment, R.id.chatListFragment
                )
        )
...
//        getSupportActionBar()?.setDisplayShowHomeEnabled(true);
    ...
    companion object {
        ...
        fun startAlone(context: Context) {
            val starter = Intent(context, BrowsePatientActivity::class.java)
